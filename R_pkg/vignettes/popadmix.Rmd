...
vignette: >
  %\VignetteIndexEntry{"Introductions for the functionis in popadmix"}
...
The input is the vector of allele frequencies (should add up to 1).

## Example
If we have the frequencies: 0.2, 0.3, 0.5 for 3 alleles, the expected heterozygosity is:
```{r}
...
The input is a matrix of allele frequencies with K rows and I columns (sums on rows must be 1) and a vector of admixture fractions gamma (must add up to 1).
...
We consider the admixture fractions (gamma): 0.3, 0.7, which means that source population 1 provides 30% of the ancestry
of the admixed population, with source population 2 providing the remaining 70%.
```{r echo=TRUE}
gamma <- c(0.3, 0.7)
```
We can calculate the $H_{adm}$ of the admixed population: 
```{r echo=TRUE}
...
The input consists of two vectors, pop.1 and pop.2, representing the allele frequencies for the same locus for populations 1 and 2.

## Example
The allele frequency at a locus is (0.3, 0.7) in population 1 and (0.4,0.6) in population 2. We can then calculate the $F_{st}$:
```{r}
get.Fst(c(0.3,0.7), c(0.4,0.6))
```

# Function to calculate $F_{st}$ between a founder population and the admixed population for the K=2 founder population case

In the following sections, we show how to calculate the $F_{st}$ between a founder population and the admixed population.
...
For the K = 2 founder population case, we can use the `get.Fst.admix` function to calculate $F_{st}$ between population 1 (pop.1) and the admixed population:
...
The input consists of the vectors pop.1, pop.2 (the allele frequencies corresponding to founder populations 1 and 2) and gamma (the ancestry fraction corresponding to population 1).

## Example
Consider the allele frequency of pop.1 is (0.3,0.7), the allele frequency of pop.2 is (0.4,0.6), and fraction $\gamma_k$ = 0.25. 

Then calculate $F_{st}$ between pop.1 and the admixed population:

```{r}
get.Fst.admix(c(0.3,0.7), c(0.4,0.6), 0.25)
```
...
With the given ancestry fractions gamma: 0.3, 0.7
```{r echo=TRUE}
gamma <- c(0.3, 0.7)
```
