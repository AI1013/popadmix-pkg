...
We have the function to calculate the heterozygosity and the code to get the allele frequencies for an admixed population. So if we combine them, then we can calculate the heterozygosity of the admixed population.
```{r}
...
Now calculate the $H_{adm}$ of the admixed population: 
```{r echo=TRUE}
...
We can use the following codes to create the plots related to $F_{st}$ and $H_{amd}$ of the data in the "allele.frequencie.no.extra" data file, and the x argument should be a list. 
```{r}
plot_HorF <- function (x,# x should be a list including the allele frequencies of three populations(EUROPE, AMERICA, AFRICA)
                       ...
                       breaks, # breaks are numeric vectors to categorize  Fst or Hamd.
                       label # the label of the legend.
){
  ...
  allele.frequencies.no.extra$x <-  as.matrix(allele.frequencies.no.extra[x])[[1]][1:3,]
  ...
  if(y == "Hadm"){
    ...
    pl <- ggtern(gamma, aes(gamma.2, gamma.1, gamma.3, value=hadm)) + geom_point(aes(color=Hadm)) + colScale 
...
```{r, message=FALSE}
load("D:/sdxjw/Documents/R working directory/allele.frequencies.no.extra.RData")
